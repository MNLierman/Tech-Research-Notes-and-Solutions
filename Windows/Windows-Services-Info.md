...
#### Most guides and articles posted regurgitate the same info that Microsoft provides. We investigate the true purpose of each Windows Service. Ultimately, our goal is to disable services and features that are not in use and the user (you) don't intend on using. Doing so decreases the attack surface, and frees up system resources.
